...
| 分析失败，提示「Agent 启动后 N 分钟内没有任何输出（LLM 后端无响应）」或反复「未返回有效配置」 | 分析中途 LLM 后端无响应。现在引擎在每次尝试前都会对网关做健康预检（进度里可见「LLM 网关预检」提示，最多等 3 分钟等后端恢复，不再把整个尝试浪费在死掉的后端上），并在 agent 层限制挂起的 LLM 流（2.5 分钟空闲超时 + 1 次重试）；首次 LLM 响应的看门狗宽限（8 分钟）也比常规卡死判定（6 分钟）更长——dsh 在第一次模型调用期间本来就没有任何输出。反复出现时请到 设置 → LLM 配置 检查 Provider 可用性/配额后重新分析。 |
| 重试好像「从头再来」/ 第 2、3 次尝试把前面做过的事又做了一遍？ | 并没有——进度流是累积的：「第 N 次尝试从这里开始」分隔线上方变暗的条目是之前尝试的历史记录。重试的提示词会带上前面尝试已确认的事实（读过的文件、选过的端口、做过的修复）；进度事件现在归属于真正产生它的那次尝试（此前一个文件跟踪缺陷会让重试的事件不可见、或把上一次尝试的命令显示成新尝试的）；上次尝试遗留的 dev 服务/构建进程（包括以前扫不掉的相对路径 `next build`/`next dev` 孤儿进程——它们曾一直写坏 `.next`）也会在下次尝试前被清扫。 |
