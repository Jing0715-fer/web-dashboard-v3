...
| 重试好像「从头再来」/ 第 2、3 次尝试把前面做过的事又做了一遍？ | 并没有——进度流是累积的：「第 N 次尝试从这里开始」分隔线上方变暗的条目是之前尝试的历史记录。重试的提示词会带上前面尝试已确认的事实（读过的文件、选过的端口、做过的修复）；上次尝试遗留的 dev 服务/构建进程（包括以前扫不掉的相对路径 `next build`/`next dev` 孤儿进程——它们曾一直写坏 `.next`）也会在下次尝试前被清扫。 |
