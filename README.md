...
| Analysis retries start over / "did attempt 2+ re-run everything?" | They don't — the progress feed is cumulative: items above the "Attempt N starts here" divider (dimmed) are earlier attempts' history, kept for context. Retry prompts carry the facts earlier attempts established (files read, ports picked, fixes applied), and orphaned dev servers/builds from a dead attempt are swept before the next one spawns — including relative-path `next build`/`next dev` leftovers that used to escape the sweep and corrupt `.next`. |
