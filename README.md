...
- **Self-path rejection** — registering or analyzing the dashboard's *own* directory is refused with a clear error. Before this guard, the analysis agent's pre-flight cleanup would read the dashboard's live `.next/dev/lock` and kill the running server — the dashboard stopped itself mid-analysis. The rejection now sits at four layers (project create, harness API, engine entry, spawn), and the path comparison is *canonicalized*: a differently-cased drive letter, a junction/symlink alias, or a sub-directory whose package.json walk-up lands on the dashboard are all recognized as the same directory on disk. The same guard exists inside the agent (v1.15+), so a remote "fetch environments" against the dashboard's own directory on that machine is refused instead of racing the live server.
...
- **Silent background operation** — every probe and spawn the dashboard and its agents perform (port checks, git polling, process kills, project starts) uses direct argv with `CREATE_NO_WINDOW` instead of `cmd.exe` pipelines. On Windows, a console-less background service otherwise allocates a console window for every `netstat`/`git`/`taskkill` child — the "terminal flashing on the desktop every 30 seconds" bug (agents v1.16+).
...
| "Re-fetch environments" on a project that points at the dashboard itself (possibly via a differently-cased path or symlink) still refuses | Same guard, working as designed — even when the stored path doesn't string-match the dashboard's directory, the canonicalized comparison recognizes it. Copy the project to a separate directory if you want it managed. |
| Remote analysis of the dashboard's own directory on a paired machine | Refused by the agent's self-guard (v1.15+). Agents that predate it answer `DASHBOARD_SELF_GUARD`-less errors or may race the live dashboard on that machine — update the agent (`git pull` + restart) there. |
| A console window flashes on the desktop every few seconds (Windows) | The machine runs code that predates v1.16: the periodic probes (netstat port checks, git update polling) went through `cmd.exe /c netstat | findstr …` pipelines, and every child of the console-less service allocates its own window. Update on that machine (`git pull` + restart) — v1.16 probes run with `CREATE_NO_WINDOW` and no shell wrapper, and the device card flags pre-v1.16 agents as outdated. |
...
| Analysis fails with "Agent 启动后 N 分钟内没有任何输出（LLM 后端无响应）" or an attempt "did not return a valid config" repeatedly | The LLM backend the dashboard proxies to was unresponsive mid-analysis. The engine now health-checks the gateway before every attempt (visible as "LLM 网关预检" notes, waits up to 3 min instead of burning an attempt into a dead backend), bounds hung LLM streams at the agent layer (2.5-min idle timeout + 1 retry), and gives the agent's *first* LLM response a longer grace (8 min) than the regular stall watchdog (6 min) — dsh is legitimately silent while its first model call is in flight. If you see this repeatedly: check the configured provider's availability/quota in Settings → LLM Configuration and re-run the analysis. |
| Analysis retries start over / "did attempt 2+ re-run everything?" | They don't — the progress feed is cumulative: items above the "Attempt N starts here" divider (dimmed) are earlier attempts' history, kept for context. Retry prompts carry the facts earlier attempts established (files read, ports picked, fixes applied), events are attributed to the attempt that actually produced them (a file-tracking bug previously made a retry's feed items invisible or re-showed the previous attempt's commands under the new number), and orphaned dev servers/builds from a dead attempt are swept before the next one spawns — including relative-path `next build`/`next dev` leftovers that used to escape the sweep and corrupt `.next`. |
| One-click pull sometimes fails with `fatal: unable to access 'https://github.com/...': OpenSSL SSL_connect: SSL_ERROR_SYSCALL in connection to github.com:443` — and works when clicked again | A transient network flake (the TLS connection to github.com is torn down mid-handshake — common on unstable links). The dashboard and agents (v1.18+) now auto-retry these failures up to 3 times with a short backoff (~1s/2.5s/5s), so the click-again step is done for you; a pull that survives a flake reports "auto-retried and succeeded". If every retry fails, the error is flagged as network-transient — just pull again in a moment. |
| Any pull failure now opens an "AI Pull Diagnosis" dialog (v1.19) | When a pull still fails after the auto-retries (or hits a conflict, auth error, broken repo…), the dashboard asks your configured LLM provider (Settings → LLM Configuration; the built-in Z.ai SDK is the fallback) to diagnose it: it names the root cause, explains it in plain language, and recommends one of the already-implemented fixes — retry, stash-then-restore, or discard-the-listed-files. **You confirm every action** (discard asks twice); the LLM never generates commands, it only picks from the fixed strategy list. If the LLM is unreachable, you land on the standard options (v1.17 conflict dialog / error toast) — never a dead end. |
